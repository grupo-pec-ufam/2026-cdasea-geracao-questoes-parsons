...
- A nota do estudante em Cálculo (número real).
- A frequência do estudante no semestre anterior, em percentual (número real).

**Como saída, o programa deve imprimir:**

- A situação: `Habilitado` (se a nota for maior ou igual a 7 e a frequência for maior ou igual a 75) ou `Nao habilitado` (caso contrário).
- A nota informada, arredondada em duas casas decimais.
...
| Nota | Frequência | Saída esperada | Observação |
|------|-----------|-----------------|------------|
| 8 | 80 | `Habilitado` / `8.0` | ramo if, ambas verdadeiras |
| 8 | 70 | `Nao habilitado` / `8.0` | ramo else, nota ok mas frequência não |
| 5 | 80 | `Nao habilitado` / `5.0` | ramo else, frequência ok mas nota não |

**Privados:**

| Nota | Frequência | Saída esperada | Observação |
|------|-----------|-----------------|------------|
| 7 | 75 | `Habilitado` / `7.0` | borda exata, ambas ≥ limite |
| 6.99 | 75 | `Nao habilitado` / `6.99` | nota logo abaixo da borda |
| 7 | 74.99 | `Nao habilitado` / `7.0` | frequência logo abaixo da borda |
| 5 | 50 | `Nao habilitado` / `5.0` | ramo else, ambas falsas |
...
O operador `and` exige que todas as condições sejam verdadeiras; `or` exige que pelo menos uma seja. O `else` nunca recebe condição — ele cobre tudo que não caiu no `if`.

## 8. Tópicos abordados

- M2 – Estruturas condicionais compostas
