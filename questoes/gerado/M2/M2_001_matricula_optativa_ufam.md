...
Título: Matricula Optativa UFAM
...
A frequencia do estudante no semestre anterior, em percentual (número real).

Como saída, o programa deve imprimir:

A situacao: Habilitado (se a nota for maior ou igual a 7 e a frequência for maior ou igual a 75) ou Nao habilitado (caso contrário).
...
nota = float(...): converte a nota lida para número real.
frequencia = float(...): converte a frequência lida para número real.
if nota >= 7 and frequencia >= 75:: a matrícula só é liberada se ambas as condições forem verdadeiras — daí o uso do and.
print("Habilitado"): executado apenas quando as duas condições são satisfeitas.
else:: cobre todos os demais casos (qualquer uma das condições falha).
print("Nao habilitado"): executado quando pelo menos uma condição falha.
print(round(nota, 2)): imprime a nota, fora do condicional.
